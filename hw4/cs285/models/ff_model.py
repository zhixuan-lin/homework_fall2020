...
class FFModel(nn.Module, BaseModel):
    ...
    def get_prediction(self, obs, acs, data_statistics):
        ...
        prediction, _ = self(obs, acs, **{k: ptu.from_numpy(v) for k, v in data_statistics.items()})# TODO(Q1) get numpy array of the predicted next-states (s_t+1)
    ...
    def update(self, observations, actions, next_observations, data_statistics):
        ...
        _, delta_normalized = self(observations, actions, **{k: ptu.from_numpy(v) for k, v in data_statistics.items()})
        loss = self.loss(delta_normalized, target)# TODO(Q1) compute the loss
        # Hint: `self(...)` returns a tuple, but you only need to use one of the
        # outputs.
